...
@pipeline("era5-soil-water", name="ERA5 Volumetric Soil Water Layer")
def era5_soil_water():
    """Download and aggregate volumetric soil water layer data
    from climate data store.
    """
    # parse configuration
    with open(f"{workspace.files_path}/pipelines/era5_soil_water/config.json") as f:
        config = json.load(f)
